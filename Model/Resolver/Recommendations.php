...
class Recommendations implements ResolverInterface
{
    ...
    public function resolve(Field $field, $context, ResolveInfo $info, array $value = null, array $args = null): array
    {
        ...
        $result = $this->repository->getRecommendations(
            strtolower((string) ($args['priority'] ?? '')),
            strtolower((string) ($args['category'] ?? '')),
            (int)    ($args['currentPage'] ?? 1),
            (int)    ($args['pageSize'] ?? 20)
        );
        $result['items'] = array_map([$this->enumFormat, 'recommendation'], $result['items'] ?? []);
        return $result;
    }
}
